...
def _lazy_load_model():
    ...
    # Pre-encode all exemplars once upon load to save ~100ms per request
    _EX_EMB = _MODEL.encode(_EXAMPLES, normalize_embeddings=True)
    return _MODEL, _EX_EMB
...
def semantic_scan(
    ...
    try:
        model, ex_emb = _lazy_load_model()
        # Encode only the query (extremely fast)
