...
_PATTERNS_URLS: List[tuple[str, re.Pattern]] = [
    ("Suspicious URL",      re.compile(r"https?://(?!(?:www\.)?(anthropic|openai|google|microsoft|github|stackoverflow|wikipedia)\.com)[^\s]{40,}")),
...
def guard_output_stream(chunk_stream):
    ...
    Checks a sliding window to block secrets and prompt leakage.
    """
    buffer = ""
    for chunk in chunk_stream:
        ...
        # Check against hard patterns (Secrets and Prompt Leak)
        for label, pat in _PATTERNS_SECRETS + _PATTERNS_PROMPT_LEAK:
            if pat.search(buffer):
                yield "\n\n[SECURITY ALERT: Output blocked due to sensitive internal exposure request.]"
                return

        # We yield chunks as they arrive for better UX, relying on the sliding check
        yield chunk
        
        if len(buffer) > 1000:
            buffer = buffer[-500:] # Keep buffer size controlled