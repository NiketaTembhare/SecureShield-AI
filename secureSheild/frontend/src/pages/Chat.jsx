...
import { api, logoutUser, API_BASE, getToken, getUserRole } from '../lib/api.js';
...
export default function Chat() {
  const [messages, setMessages] = useState(() => {
    ...
    return [{ role: 'assistant', content: 'Welcome to SecureShield AI. How can I help you today?' }];
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      while (true) {
        ...
        const text = partialLine + chunkStr;
        const lines = text.split('\n');
        
        // The last element is potentially an incomplete line, save it for the next chunk
            ...
            if (data.type === 'control') {
              ...
              setMessages((prev) => {
                ...
                if (lastMsg && lastMsg.role === 'assistant') {
                  ...
                  newArr[lastIdx] = { ...lastMsg, content: lastMsg.content + data.text };
                }
                return newArr;
              });
              ...
              setMessages((prev) => {
                const newArr = [...prev];
                const lastIdx = newArr.length - 1;
                // If we have an empty assistant placeholder, replace it with the error message
                if (newArr[lastIdx] && newArr[lastIdx].role === 'assistant' && !newArr[lastIdx].content) {
                  newArr[lastIdx] = { role: 'system_alert', content: `[GATEWAY ERROR] ${data.message}` };
                  return newArr;
                }
                return [...prev, { role: 'system_alert', content: `[GATEWAY ERROR] ${data.message}` }];
              });
            }
          } catch (e) {
  ...
  const handleDashboardClick = (e) => {
    ...
    if (role !== 'admin') {
      e.preventDefault();
      setMessages((prev) => [...prev, { role: 'system_alert', content: '[SYSTEM] Access Denied. Corporate Dashboard Analytics are restricted to Administrative Personnel only.' }]);
    }
  };
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-0 hidden md:flex md:w-0'} shrink-0 transition-all duration-300 ease-in-out bg-[#0a1120]/90 backdrop-blur-md border-r border-cyan-500/30 shadow-[4px_0_15px_rgba(0,255,204,0.05)] flex flex-col z-20`}>
        {isSidebarOpen && (
          <div className="flex flex-col h-full w-64 p-4 pb-0">
            ...
            <Link onClick={handleDashboardClick} className="flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-cyan-900/30 hover:shadow-[inset_0_0_10px_rgba(0,255,204,0.1)] border border-transparent hover:border-cyan-500/30 text-sm transition-all text-cyan-100" to="/dashboard">
